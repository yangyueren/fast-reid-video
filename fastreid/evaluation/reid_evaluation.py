...
class ReidEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        def extract_frame_idx(pids):
            frames = []
            idxs = []
            for pid in pids:
                frame, idx = map(int, pid.split('_'))
                frames.append(frame)
                idxs.append(idx)
            return frames, idxs
        ...
        EXTRACT = False
        if EXTRACT:
            #一次性提取carlavehicle的queryfeature和carla camera的testfeature
            #test格式 c000-007758-003.jpg camid, frameid, idxinframe
            frames, idxs = extract_frame_idx(pids)
            # query feature, person ids and camera ids
            query_features = features[:self._num_query]
            query_pids = np.asarray(frames[:self._num_query])
            # query_idxs = np.asarray(idxs[:self._num_query]) # no use
            query_camids = np.asarray(camids[:self._num_query])

            # gallery features, person ids and camera ids
            gallery_features = features[self._num_query:]
            gallery_frames = np.asarray(frames[self._num_query:])
            gallery_idxs = np.asarray(idxs[self._num_query:])
            ...
            np.save(file_name+'carla_idx_in_frame_2048.npy', gallery_idxs)
            np.save(file_name+'carla_camid_2048.npy', gallery_camids)


            gallery_pids = gallery_frames
            ...
            # np.save(file_name+'camid_2048.npy', gallery_camids)

            np.save(file_name+'query_gf_2048.npy', query_features.numpy())
            np.save(file_name+'query_pid_2048.npy', query_pids)
            np.save(file_name+'query_camid_2048.npy', query_camids)
